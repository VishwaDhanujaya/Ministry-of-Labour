...
function renderPaginationButtons(totalPages) {
    ...
    if (totalPages <= 1) {
        container.innerHTML = `
            <button disabled class="px-3.5 py-2 border border-gray-200 text-gray-400 rounded-xl text-xs cursor-not-allowed bg-gray-50/50">` + '<?= t("pagination_prev", "Prev") ?>' + `</button>
            <button class="px-3.5 py-2 bg-primary text-white font-bold rounded-xl text-xs shadow-sm">1</button>
            <button disabled class="px-3.5 py-2 border border-gray-200 text-gray-400 rounded-xl text-xs cursor-not-allowed bg-gray-50/50">` + '<?= t("pagination_next", "Next") ?>' + `</button>
        `;
        return;
    }
