...
<div class="group bg-white rounded-2xl overflow-hidden shadow-2xl shadow-gray-200/60 border-[0.5px] border-[#D4D4D4] w-full max-w-[491px] h-[600px] relative">
    ...
    <!-- Hover Overlay -->
    <div class="absolute inset-0 bg-primary/50 backdrop-blur-[2px] opacity-0 group-hover:opacity-100 transition-all duration-300 flex flex-col sm:flex-row items-center justify-center gap-3 px-4 z-20">
        <!-- Fullscreen / Preview Icon -->
        <button type="button" data-pdf-url="<?php echo htmlspecialchars($pdfUrl); ?>" data-pdf-title="<?php echo htmlspecialchars($pdfTitle); ?>" class="open-pdf-modal bg-white text-primary px-4 py-2.5 sm:px-5 sm:py-2.5 rounded-xl font-semibold text-xs sm:text-sm hover:bg-gray-100 transition-colors shadow-md flex items-center justify-center gap-2 whitespace-nowrap max-w-[200px] transform translate-y-4 group-hover:translate-y-0 duration-300 cursor-pointer">
            ...
            <span class="notranslate"><?= t('open_pdf', 'Open PDF') ?></span>
        </button>
        <!-- Download Icon -->
        <a href="<?php echo htmlspecialchars($pdfUrl); ?>" download aria-label="Download PDF" class="bg-primary text-white px-4 py-2.5 sm:px-5 sm:py-2.5 rounded-xl font-semibold text-xs sm:text-sm hover:bg-primary/90 transition-colors shadow-md flex items-center justify-center gap-2 whitespace-nowrap max-w-[200px] transform translate-y-4 group-hover:translate-y-0 duration-300 delay-75">
            <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="shrink-0">
                <path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"></path>
                <polyline points="7 10 12 15 17 10"></polyline>
                <line x1="12" y1="15" x2="12" y2="3"></line>
            </svg>
            <span class="notranslate"><?= t('download', 'Download') ?></span>
        </a>
    </div>
</div>
