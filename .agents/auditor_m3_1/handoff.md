...
**Verification Method**
- Inspect `c:\xampp\htdocs\Ministry-of-Labour\contact-us.php` lines 393-470 using `view_file`.
- Note the absence of `window.showToast()` and the presence of `alert()` fallbacks.
- Note that `document.getElementById('toast')` is null because it was removed from `includes/footer.php`.
