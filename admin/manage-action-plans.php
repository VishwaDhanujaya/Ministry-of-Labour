...
if (isset($_GET['delete'])) {
    ...
    $stmt = $pdo->prepare("SELECT pdf_path, pdf_path_si, pdf_path_ta FROM action_plans WHERE id = ?");
    $stmt->execute([$del_id]);
    $pub = $stmt->fetch();
    ...
    if (empty($title)) {
        ...
        if ($action === 'add') {
            ...
            if (isset($_FILES['pdf_file_si']) && $_FILES['pdf_file_si']['error'] === UPLOAD_ERR_OK) {
                $uploadResult = handleFileUpload($_FILES['pdf_file_si'], 'uploads/action_plans', ['application/pdf'], 5242880);
                if ($uploadResult['success']) $pdf_path_si = $uploadResult['path'];
                else $error = $uploadResult['error'];
            }
            if (isset($_FILES['pdf_file_ta']) && $_FILES['pdf_file_ta']['error'] === UPLOAD_ERR_OK) {
                $uploadResult = handleFileUpload($_FILES['pdf_file_ta'], 'uploads/action_plans', ['application/pdf'], 5242880);
                if ($uploadResult['success']) $pdf_path_ta = $uploadResult['path'];
                else $error = $uploadResult['error'];
            }
            if (empty($error)) {
                $stmt = $pdo->prepare("INSERT INTO action_plans (title, description, pdf_path, pdf_path_si, pdf_path_ta, status) VALUES (?, ?, ?, ?, ?, ?)");
            ...
            if (!$existing) {
                ...
                if (empty($error)) {
                    $stmt = $pdo->prepare("UPDATE action_plans SET title = ?, description = ?, pdf_path = ?, pdf_path_si = ?, pdf_path_ta = ?, status = ? WHERE id = ?");
                ...
                <div class="p-6 overflow-y-auto">
                    <form id="pubForm" action="" method="POST" enctype="multipart/form-data" class="js-validate-form space-y-6">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-5">
                            <div>
                                <label class="block text-[11px] font-bold text-slate-500 uppercase tracking-wider mb-2" id="pdfLabelEn">PDF File (English)</label>
                            ...
                            <div>
                                <label class="block text-[11px] font-bold text-slate-500 uppercase tracking-wider mb-2" id="pdfLabelSi">PDF File (Sinhala)</label>
                            ...
                            <div>
                                <label class="block text-[11px] font-bold text-slate-500 uppercase tracking-wider mb-2" id="pdfLabelTa">PDF File (Tamil)</label>
