...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'delete_pdf_ajax') {
    ...
    $stmt = $pdo->prepare("SELECT $col FROM action_plans WHERE id = ?");
    $stmt->execute([$id]);
    $existing = $stmt->fetch();
    ...
    $stmt = $pdo->prepare("UPDATE action_plans SET $col = NULL WHERE id = ?");
    $stmt->execute([$id]);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
    ...
    if (empty($title)) {
        ...
        if ($action === 'add') {
            ...
            if (empty($error)) {
                $stmt = $pdo->prepare("INSERT INTO action_plans (title, title_si, title_ta, description, description_si, description_ta, pdf_path, pdf_path_si, pdf_path_ta, status) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
            ...
            if (!$existing) {
                ...
                if (empty($error)) {
                    $stmt = $pdo->prepare("UPDATE action_plans SET title = ?, title_si = ?, title_ta = ?, description = ?, description_si = ?, description_ta = ?, pdf_path = ?, pdf_path_si = ?, pdf_path_ta = ?, status = ? WHERE id = ?");
