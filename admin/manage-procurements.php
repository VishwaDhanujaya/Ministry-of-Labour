...
if (isset($_GET['delete'])) {
    ...
    $stmt = $pdo->prepare("SELECT pdf_path, pdf_path_si, pdf_path_ta FROM procurements WHERE id = ?");
    $stmt->execute([$del_id]);
    $proc = $stmt->fetch();
    ...
    if (empty($title)) {
        ...
        if ($action === 'add') {
            ...
            if (isset($_FILES['pdf_file_si']) && $_FILES['pdf_file_si']['error'] === UPLOAD_ERR_OK) {
                $uploadResult = handleFileUpload($_FILES['pdf_file_si'], 'uploads/procurements', ['application/pdf'], 5242880);
                if ($uploadResult['success']) $pdf_path_si = $uploadResult['path'];
                else $error = $uploadResult['error'];
            }
            if (isset($_FILES['pdf_file_ta']) && $_FILES['pdf_file_ta']['error'] === UPLOAD_ERR_OK) {
                $uploadResult = handleFileUpload($_FILES['pdf_file_ta'], 'uploads/procurements', ['application/pdf'], 5242880);
