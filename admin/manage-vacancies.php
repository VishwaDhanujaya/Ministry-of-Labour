...
requireLogin();
requirePermission('manage_vacancies');
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'delete_pdf_ajax') {
    ...
    $stmt = $pdo->prepare("SELECT $col FROM vacancies WHERE id = ?");
    $stmt->execute([$id]);
    $existing = $stmt->fetch();
    ...
    $stmt = $pdo->prepare("UPDATE vacancies SET $col = NULL WHERE id = ?");
    $stmt->execute([$id]);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
    ...
    if (empty($title)) {
        ...
        if ($action === 'add') {
            ...
            if (empty($error)) {
                $stmt = $pdo->prepare("INSERT INTO vacancies (title, title_si, title_ta, description, description_si, description_ta, pdf_path, pdf_path_si, pdf_path_ta, status) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
            ...
            if (!$existing) {
                ...
                if (empty($error)) {
                    $stmt = $pdo->prepare("UPDATE vacancies SET title = ?, title_si = ?, title_ta = ?, description = ?, description_si = ?, description_ta = ?, pdf_path = ?, pdf_path_si = ?, pdf_path_ta = ?, status = ? WHERE id = ?");
...
<div class="flex-1 flex flex-col min-w-0 bg-[#F8F9FA] relative z-10 font-inter">
    ...
    <main class="flex-1 overflow-x-hidden overflow-y-auto p-4 md:p-8">
        ...
        function openAddModal() {
            ...
            // Reset language tabs
            const firstTab = document.querySelector('.lang-tab-btn[data-target="vacancy-tabs-en"]');
