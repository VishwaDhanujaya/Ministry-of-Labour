...
function renderAdminTable(array $headers, array $rows, callable $renderRowCallback, array $options = []) {
    ...
    $containerClass = $options['containerClass'] ?? 'bg-white rounded-2xl border border-slate-100 shadow-[0_4px_16px_rgba(0,0,0,0.015)] overflow-hidden mb-12';
    $pagination = $options['pagination'] ?? null;
    ?>
    <div class="<?= htmlspecialchars($containerClass) ?>">
        <div class="overflow-x-auto">
            <table class="<?= htmlspecialchars($tableClass) ?>" style="min-width: <?= htmlspecialchars($minWidth) ?>;">
                <thead>
                    <tr class="bg-slate-50/50 text-slate-400 border-b border-slate-100 text-[11px] font-bold uppercase tracking-wider select-none">
                ...
                <?php
                $tbodyClass = $options['tbodyClass'] ?? 'divide-y divide-slate-50 text-[13px]';
        ...
        ?>
            <!-- Pagination -->
            <div class="px-6 py-4 border-t border-slate-50 flex items-center justify-between font-inter bg-slate-50/30">
                <p class="text-[12.5px] text-slate-500 font-medium">Showing <?= htmlspecialchars($showingCount) ?> of <?= htmlspecialchars($totalItems) ?> entries</p>
                <div class="flex gap-1.5">
                    <button class="px-3.5 py-1.5 border border-slate-200 rounded-xl text-[12px] font-bold text-slate-400 bg-white cursor-not-allowed" disabled>Prev</button>
                    <button class="px-3.5 py-1.5 bg-[#13273F] text-white rounded-xl text-[12px] font-bold shadow-sm">1</button>
                    <button class="px-3.5 py-1.5 border border-slate-200 rounded-xl text-[12px] font-bold text-slate-400 bg-white cursor-not-allowed" disabled>Next</button>
                </div>
            </div>
        <?php endif; ?>
    </div>
