...
if (isset($_GET['delete'])) {
    ...
    $stmt = $pdo->prepare("SELECT pdf_path FROM rti_reports WHERE id = ?");
    $stmt->execute([$del_id]);
    $pub = $stmt->fetch();
        ...
        if ($action === 'add') {
            if (!isset($_FILES['pdf_file']) || $_FILES['pdf_file']['error'] !== UPLOAD_ERR_OK) {
                ...
                if ($uploadResult['success']) {
                    ...
                    $stmt = $pdo->prepare("INSERT INTO rti_reports (title, description, pdf_path, status) VALUES (?, ?, ?, ?)");
            ...
            if (!$existing) {
                ...
                if (empty($error)) {
                    $stmt = $pdo->prepare("UPDATE rti_reports SET title = ?, description = ?, pdf_path = ?, status = ? WHERE id = ?");
