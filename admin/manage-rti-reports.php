...
if (isset($_GET['delete'])) {
    ...
    $stmt = $pdo->prepare("SELECT pdf_path, pdf_path_si, pdf_path_ta FROM rti_reports WHERE id = ?");
    $stmt->execute([$del_id]);
    $pub = $stmt->fetch();
    ...
    if ($pub) {
        ...
        $stmt = $pdo->prepare("DELETE FROM rti_reports WHERE id = ?");
        $stmt->execute([$del_id]);
        $success = "RTI Report deleted successfully.";
    } else {
        $error = "RTI Report not found.";
    }
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'delete_pdf_ajax') {
    ...
    $stmt = $pdo->prepare("SELECT $col FROM rti_reports WHERE id = ?");
    $stmt->execute([$id]);
    $existing = $stmt->fetch();
    ...
    $stmt = $pdo->prepare("UPDATE rti_reports SET $col = NULL WHERE id = ?");
    $stmt->execute([$id]);
    ...
    if (empty($title)) {
        ...
        if ($action === 'add') {
            if (isset($_FILES['pdf_file']) && $_FILES['pdf_file']['error'] === UPLOAD_ERR_OK) {
                $uploadResult = handleFileUpload($_FILES['pdf_file'], 'uploads/rti_reports', ['application/pdf'], 5242880);
            ...
            if (isset($_FILES['pdf_file_si']) && $_FILES['pdf_file_si']['error'] === UPLOAD_ERR_OK) {
                $uploadResult = handleFileUpload($_FILES['pdf_file_si'], 'uploads/rti_reports', ['application/pdf'], 5242880);
                if ($uploadResult['success']) $pdf_path_si = $uploadResult['path'];
                else $error = $uploadResult['error'];
            }
            if (isset($_FILES['pdf_file_ta']) && $_FILES['pdf_file_ta']['error'] === UPLOAD_ERR_OK) {
                $uploadResult = handleFileUpload($_FILES['pdf_file_ta'], 'uploads/rti_reports', ['application/pdf'], 5242880);
                if ($uploadResult['success']) $pdf_path_ta = $uploadResult['path'];
                else $error = $uploadResult['error'];
            }
            if (empty($error)) {
                $stmt = $pdo->prepare("INSERT INTO rti_reports (title, title_si, title_ta, description, description_si, description_ta, pdf_path, pdf_path_si, pdf_path_ta, status) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
                if ($stmt->execute([$title, $title_si, $title_ta, $description, $description_si, $description_ta, $pdf_path, $pdf_path_si, $pdf_path_ta, $status])) {
                    $success = "RTI Report added successfully.";
                } else {
                    $error = "Failed to add rti_report.";
                }
            }
        } elseif ($action === 'edit') {
            ...
            if (!$existing) {
                ...
                if (empty($error)) {
                    $stmt = $pdo->prepare("UPDATE rti_reports SET title = ?, title_si = ?, title_ta = ?, description = ?, description_si = ?, description_ta = ?, pdf_path = ?, pdf_path_si = ?, pdf_path_ta = ?, status = ? WHERE id = ?");
                    if ($stmt->execute([$title, $title_si, $title_ta, $description, $description_si, $description_ta, $pdf_path, $pdf_path_si, $pdf_path_ta, $status, $edit_id])) {
                        $success = "RTI Report updated successfully.";
                    } else {
                        $error = "Failed to update rti_report.";
                    }
                }
            }
        }
...
// Fetch RTI Reports
$stmt = $pdo->query("SELECT * FROM rti_reports ORDER BY created_at DESC");
$rti_reports = $stmt->fetchAll();

$pageTitle = 'Manage RTI Reports';
...
<div class="flex-1 flex flex-col min-w-0 bg-[#F8F9FA] relative z-10 font-inter">
    ...
    <main class="flex-1 overflow-x-hidden overflow-y-auto p-4 md:p-8 bg-[#F8F9FA]">
        ...
        <div class="flex flex-col sm:flex-row justify-between sm:items-center gap-4 mb-8">
            <div>
                <h2 class="text-3xl font-extrabold font-montserrat text-slate-800 tracking-tight">Manage RTI Reports</h2>
                <p class="text-[13px] text-slate-500 mt-1 font-inter">Publish and manage RTI Reports and resource documents.</p>
            </div>
        ...
        renderAdminTable($headers, $rti_reports, function($pub) {
            ...
            ?>
            <tr class="hover:bg-slate-50/60 bg-white border-b border-slate-50/70 transition-all duration-150 group cursor-pointer" onclick="showPreviewModal(<?= $pub['id'] ?>, '<?= htmlspecialchars(addslashes($pub['title'])) ?>', 'manage-rti-reports?delete=<?= $pub['id'] ?>&csrf_token=<?= generateCsrfToken() ?>', <?= htmlspecialchars(json_encode($pub, JSON_HEX_APOS | JSON_HEX_QUOT)) ?>)">
                <td class="py-4 px-6">
            ...
            'minWidth' => '800px',
            'emptyTitle' => 'No RTI Reports found',
            ...
            'pagination' => [
                'total_items' => count($rti_reports),
                'showing_count' => count($rti_reports),
                'per_page' => 10,
                'enable_paging' => true
            ]
        ]);
                    ...
                    <form id="pubForm" action="" method="POST" enctype="multipart/form-data" class="js-validate-form space-y-6">
                        ...
                        <?php renderTrilingualInputFields([
                            ...
                            'fields' => [
                                ...
                                [
                                    ...
                                    'placeholder' => 'RTI Report description'
                                ]
                            ]
                        ]); ?>
                        ...
                        <div class="grid grid-cols-1 gap-5 mt-5">
                            <div>
                                ...
                                <div class="relative">
                                    <select name="status" id="pubStatus" class="w-full px-4 py-2.5 bg-slate-50 border border-slate-200/80 rounded-xl focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary text-[13px] text-slate-700 font-semibold appearance-none cursor-pointer">
        ...
        function openAddModal() {
            ...
            document.getElementById('modalTitle').innerHTML = '<svg class="w-5 h-5 mr-2 text-secondary" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6.253v13m0-13C10.832 5.477 9.246 5 7.5 5S4.168 5.477 3 6.253v13C4.168 18.477 5.754 18 7.5 18s3.332.477 4.5 1.253m0-13C13.168 5.477 14.754 5 16.5 5c1.747 0 3.332.477 4.5 1.253v13C19.832 18.477 18.247 18 16.5 18c-1.746 0-3.332.477-4.5 1.253"></path></svg> Add New RTI Report';
            ...
            document.getElementById('submitBtnText').textContent = 'Create RTI Report';
