                    ...
                    <tr class="hover:bg-gray-50 transition-colors group cursor-pointer" onclick="showPreviewModal(<?= $gallery['id'] ?>, '<?= htmlspecialchars(addslashes($gallery['title'])) ?>', 'new-gallery.php?id=<?= $gallery['id'] ?>', 'gallery.php?delete=<?= $gallery['id'] ?>&csrf_token=<?= generateCsrfToken() ?>')">
                        ...
                        <td class="py-4 px-6">
                            <div class="flex items-center space-x-2 opacity-0 group-hover:opacity-100 transition-opacity">
                                ...
                                <a href="gallery.php?delete=<?= $gallery['id'] ?>&csrf_token=<?= generateCsrfToken() ?>" onclick="event.stopPropagation(); return confirm('Are you sure you want to delete this album and all its images?');" class="js-delete-row p-1.5 text-gray-400 hover:text-red-500 transition-colors" title="Delete">
