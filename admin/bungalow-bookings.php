                        ...
                        <div class="js-booking-card rounded-xl border border-gray-200 border-t-4 <?= $borderAccent ?> overflow-hidden shadow-sm hover:shadow-md bg-white flex flex-col transition-all duration-200 relative group">
                            ...
                            <div class="bg-gray-50/80 p-4 border-t border-gray-100 flex gap-2.5">
                                ...
                                <?php if ($booking['status'] === 'Pending'): ?>
                                    <a href="bungalow-bookings.php?action=reject&id=<?= $booking['id'] ?>" onclick="return confirm('Are you sure you want to cancel this booking?');" class="px-3 py-2 bg-white border border-red-200 text-red-600 rounded-md font-bold text-[12px] hover:bg-red-50 transition-all shadow-sm" title="Reject">
                                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"></path></svg>
                                    </a>
                                    <a href="bungalow-bookings.php?action=approve&id=<?= $booking['id'] ?>" onclick="return confirm('Are you sure you want to approve this booking?');" class="flex-1 py-2 bg-[#0A6C5B] text-white text-center rounded-md font-bold text-[12px] hover:bg-[#085a4c] transition-all shadow-sm">Approve</a>
                                <?php endif; ?>
                                
                                <?php if ($booking['status'] === 'Confirmed'): ?>
                                    <a href="bungalow-bookings.php?action=reject&id=<?= $booking['id'] ?>" onclick="return confirm('Are you sure you want to cancel this confirmed booking?');" class="flex-1 py-2 bg-white border border-red-200 text-red-600 text-center rounded-md font-bold text-[12px] hover:bg-red-50 transition-all shadow-sm">Cancel Booking</a>
                                <?php endif; ?>

                                <?php if ($booking['status'] === 'Cancelled'): ?>
                                    <a href="bungalow-bookings.php?action=delete&id=<?= $booking['id'] ?>" onclick="return confirm('Are you sure you want to permanently delete this cancelled booking?');" class="flex-1 py-2 bg-white border border-gray-300 text-gray-600 text-center rounded-md font-bold text-[12px] hover:bg-gray-100 hover:text-gray-900 transition-all shadow-sm">Delete Record</a>
                                <?php endif; ?>
                            </div>
                        </div>
                        <?php endforeach; ?>
