...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'delete_pdf_ajax') {
    ...
    $stmt = $pdo->prepare("SELECT $col FROM special_notices WHERE id = ?");
    $stmt->execute([$id]);
    $existing = $stmt->fetch();
    ...
    $stmt = $pdo->prepare("UPDATE special_notices SET $col = NULL WHERE id = ?");
    $stmt->execute([$id]);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
    ...
    if (empty($title)) {
        ...
        if (empty($error)) {
            if ($action === 'add') {
                $stmt = $pdo->prepare("INSERT INTO special_notices (title, title_si, title_ta, content, content_si, content_ta, status, pdf_path, pdf_path_si, pdf_path_ta) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
                ...
                // Fetch current paths to avoid overwriting with nulls if no new file is uploaded
                $stmt = $pdo->prepare("SELECT pdf_path, pdf_path_si, pdf_path_ta FROM special_notices WHERE id = ?");
                $stmt->execute([$edit_id]);
                $existing = $stmt->fetch();
                if ($existing) {
                    if ($pdf_path === null) $pdf_path = $existing['pdf_path'];
                    if ($pdf_path_si === null) $pdf_path_si = $existing['pdf_path_si'];
                    if ($pdf_path_ta === null) $pdf_path_ta = $existing['pdf_path_ta'];
                }
                
                $stmt = $pdo->prepare("UPDATE special_notices SET title = ?, title_si = ?, title_ta = ?, content = ?, content_si = ?, content_ta = ?, status = ?, pdf_path = ?, pdf_path_si = ?, pdf_path_ta = ? WHERE id = ?");
...
$pageTitle = 'Manage Special Notices';
...
<div class="flex-1 flex flex-col min-w-0 bg-[#F8F9FA] relative z-10 font-inter">
    ...
    <main class="flex-1 overflow-x-hidden overflow-y-auto p-4 md:p-8 bg-[#F8F9FA]">
        ...
        renderAdminTable($headers, $notices, function($notice) {
            $statusClass = $notice['status'] === 'Published' ? 'bg-green-50 text-green-700 border-green-200' : 'bg-orange-50 text-orange-700 border-orange-200';
            ?>
            <tr class="hover:bg-slate-50/60 bg-white border-b border-slate-50/70 transition-all duration-150 group cursor-pointer" onclick="showPreviewModal(<?= $notice['id'] ?>, '<?= htmlspecialchars(addslashes($notice['title'])) ?>', 'manage-special-notices?delete=<?= $notice['id'] ?>&csrf_token=<?= generateCsrfToken() ?>', <?= htmlspecialchars(json_encode($notice, JSON_HEX_APOS | JSON_HEX_QUOT)) ?>)">
                <td class="py-4 px-6">
                    ...
                    <div id="preview-content-<?= $notice['id'] ?>" class="hidden">
                        <div class="flex flex-col gap-4">
                            <div class="flex flex-wrap gap-2">
                                <span class="px-2.5 py-1 rounded-md text-[11px] font-bold border shadow-sm <?= $statusClass ?>"><?= htmlspecialchars($notice['status']) ?></span>
                                <span class="px-2.5 py-1 bg-gray-50 text-gray-600 border border-slate-100 text-[11px] font-bold rounded-md shadow-sm uppercase tracking-wider"><?= date('M d, Y', strtotime($notice['created_at'])) ?></span>
                            </div>
                            <?php if (!empty($notice['content'])): ?>
                ...
                <td class="py-4 px-6 text-[13px] text-slate-400 font-mono"><?= date('M d, Y', strtotime($notice['created_at'])) ?></td>
                <td class="py-4 px-6 text-right" onclick="event.stopPropagation();">
                    <div class="flex items-center justify-end gap-1.5">
                        <button onclick='openEditModal(<?= json_encode($notice, JSON_HEX_APOS | JSON_HEX_QUOT) ?>)' class="w-8.5 h-8.5 rounded-xl bg-slate-50 border border-slate-100 hover:bg-slate-100 hover:text-slate-800 text-slate-400 flex items-center justify-center transition-all shadow-sm" title="Edit">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M16.862 4.487l1.687-1.688a1.875 1.875 0 112.652 2.652L10.582 16.07a4.5 4.5 0 01-1.897 1.13L6 18l.8-2.685a4.5 4.5 0 011.13-1.897l8.932-8.931zm0 0L19.5 7.125M18 14v4.75A2.25 2.25 0 0115.75 21H5.25A2.25 2.25 0 013 18.75V8.25A2.25 2.25 0 015.25 6H10"></path></svg>
                        </button>
                        <a href="manage-special-notices?delete=<?= $notice['id'] ?>&csrf_token=<?= generateCsrfToken() ?>" data-confirm="Are you sure you want to delete this special notice?" class="w-8.5 h-8.5 rounded-xl bg-rose-50/50 border border-rose-100/50 hover:bg-rose-50 hover:text-rose-600 text-rose-400 flex items-center justify-center transition-all shadow-sm" title="Delete">
            ...
            'minWidth' => '800px',
            'emptyTitle' => 'No special notices found',
            'emptySubtitle' => 'There are no special notices matching your criteria.',
            'filters' => [
        ...
        <div id="noticeModal" class="fixed inset-0 z-[150] hidden items-center justify-center bg-black/50 backdrop-blur-sm p-4">
            <div class="bg-white rounded-xl shadow-xl w-full max-w-2xl overflow-hidden transform transition-all flex flex-col max-h-[90vh]">
                ...
                <div class="p-6 overflow-y-auto">
                    <form id="noticeForm" action="" method="POST" enctype="multipart/form-data" class="js-validate-form space-y-6">
                        <input type="hidden" name="csrf_token" value="<?= generateCsrfToken() ?>">
                        <input type="hidden" name="action" id="formAction" value="add">
                        <input type="hidden" name="notice_id" id="noticeId" value="">
                        
                        <?php renderTrilingualInputFields([
                            'tab_group_id' => 'notice-tabs',
                            'fields' => [
                                [
                                    'name' => 'title',
                                    'label' => 'Title',
                                    'type' => 'input',
                                    'id_prefix' => 'noticeTitle',
                                    'required' => true,
                        ...
                        <?php renderTrilingualPdfUploadFields([
                            ...
                            'file_input_id_prefix' => 'noticePdf',
        ...
        function openAddModal() {
            ...
            document.getElementById('noticeTitleSi').value = '';
            document.getElementById('noticeTitleTa').value = '';
            
            if (window.quillnoticeContentEn) window.quillnoticeContentEn.setText('');
            if (window.quillnoticeContentSi) window.quillnoticeContentSi.setText('');
            if (window.quillnoticeContentTa) window.quillnoticeContentTa.setText('');
            ...
            // Reset language tabs
            const firstTab = document.querySelector('.lang-tab-btn[data-target="notice-tabs-en"]');
        ...
        function openEditModal(notice) {
            ...
            document.getElementById('noticeTitleEn').value = notice.title;
            document.getElementById('noticeTitleSi').value = notice.title_si || '';
            document.getElementById('noticeTitleTa').value = notice.title_ta || '';
            
            if (window.quillnoticeContentEn) window.quillnoticeContentEn.root.innerHTML = notice.content || '';
            if (window.quillnoticeContentSi) window.quillnoticeContentSi.root.innerHTML = notice.content_si || '';
            if (window.quillnoticeContentTa) window.quillnoticeContentTa.root.innerHTML = notice.content_ta || '';
            ...
            if (notice.pdf_path) {
                document.getElementById('pdfLinkEn').href = window.resolvePdfUrlJs(notice.pdf_path);
            ...
            if (notice.pdf_path_si) {
                document.getElementById('pdfLinkSi').href = window.resolvePdfUrlJs(notice.pdf_path_si);
            ...
            if (notice.pdf_path_ta) {
                document.getElementById('pdfLinkTa').href = window.resolvePdfUrlJs(notice.pdf_path_ta);
