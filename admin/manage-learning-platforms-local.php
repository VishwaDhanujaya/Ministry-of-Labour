...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
    ...
    if (empty($title)) {
        ...
        if ($action === 'add') {
            ...
            if (empty($error)) {
                $stmt = $pdo->prepare("INSERT INTO learning_platforms_local (title, description, pdf_path, pdf_path_si, pdf_path_ta, status) VALUES (?, ?, ?, ?, ?, ?)");
            ...
            if (!$existing) {
                ...
                if (empty($error)) {
                    $stmt = $pdo->prepare("UPDATE learning_platforms_local SET title = ?, description = ?, pdf_path = ?, pdf_path_si = ?, pdf_path_ta = ?, status = ? WHERE id = ?");
                    ...
                    <form id="pubForm" action="" method="POST" enctype="multipart/form-data" class="js-validate-form space-y-6">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                ...
                                <input type="text" name="title" id="pubTitle" required placeholder="Local Learning Platform title" class="w-full px-4 py-3 bg-[#F9FAFB] border border-gray-100 rounded-lg focus:outline-none focus:ring-1 focus:ring-secondary text-[13px] text-gray-900 placeholder-gray-400">
                            </div>
                            <div>
