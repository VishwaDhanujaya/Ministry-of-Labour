...
requireLogin();
requirePermission('manage_local_pubs');
...
if (isset($_GET['delete'])) {
    ...
    $stmt = $pdo->prepare("SELECT pdf_path, pdf_path_si, pdf_path_ta FROM learning_platforms_local WHERE id = ?");
    $stmt->execute([$del_id]);
    $pub = $stmt->fetch();
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'delete_pdf_ajax') {
    ...
    $stmt = $pdo->prepare("SELECT $col FROM learning_platforms_local WHERE id = ?");
    $stmt->execute([$id]);
    $existing = $stmt->fetch();
    ...
    $stmt = $pdo->prepare("UPDATE learning_platforms_local SET $col = NULL WHERE id = ?");
    $stmt->execute([$id]);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
    ...
    if (empty($title)) {
        ...
        if ($action === 'add') {
            ...
            if (empty($error)) {
                $stmt = $pdo->prepare("INSERT INTO learning_platforms_local (title, title_si, title_ta, description, description_si, description_ta, pdf_path, pdf_path_si, pdf_path_ta, status) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
            ...
            if (!$existing) {
                ...
                if (empty($error)) {
                    $stmt = $pdo->prepare("UPDATE learning_platforms_local SET title = ?, title_si = ?, title_ta = ?, description = ?, description_si = ?, description_ta = ?, pdf_path = ?, pdf_path_si = ?, pdf_path_ta = ?, status = ? WHERE id = ?");
...
$pageTitle = 'Manage Local Learning Platforms';
...
<div class="flex-1 flex flex-col min-w-0 bg-[#F8F9FA] relative z-10 font-inter">
    ...
    <main class="flex-1 overflow-x-hidden overflow-y-auto p-4 md:p-8 bg-[#F8F9FA]">
        ...
        <div class="flex flex-col sm:flex-row justify-between sm:items-center gap-4 mb-8">
            <div>
                <h2 class="text-3xl font-extrabold font-montserrat text-slate-800 tracking-tight">Manage Local Learning Platforms</h2>
        ...
        renderAdminTable($headers, $learning_platforms_local, function($pub) {
            ...
            'minWidth' => '800px',
            'emptyTitle' => 'No Local Learning Platforms found',
            ...
            'pagination' => [
                'total_items' => count($learning_platforms_local),
                'showing_count' => count($learning_platforms_local),
                'per_page' => 10,
                'enable_paging' => true
            ]
        ]);
                        ...
                        <?php renderTrilingualInputFields([
                            ...
                            'fields' => [
                                [
                                    ...
                                    'id_prefix' => 'pubTitle',
                                    'required' => true,
                                    'placeholder' => 'Local Learning Platform title'
                                ],
                                [
                                    'name' => 'description',
                                    'label' => 'Description',
                                    ...
                                    'placeholder' => 'Local Learning Platform description'
                                ]
                            ]
                        ]); ?>
