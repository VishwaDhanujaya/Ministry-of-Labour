...
function updatePaginationUI(itemsPerPage) {
    ...
    // Update summary text
    updatePaginationSummary(startIdx, endIdx, totalItems, 'downloads');
}
...
function updatePaginationSummary(startIdx, endIdx, totalItems, entityType = 'updates') {
    ...
    const entityNames = {
        ...
        downloads: { en: 'downloads', si: 'බාගත කිරීම්', ta: 'பதிவிறக்கங்கள்' },
        publications: { en: 'publications', si: 'ප්‍රකාශන', ta: 'வெளியீடுகள்' }
    };

    const entity = entityNames[entityType] || entityNames.downloads;
    ...
    if (lang === 'si') {
        ...
        const singularName = entityType === 'vacancies' ? 'vacancy' : (entityType === 'notices' ? 'notice' : (entityType === 'downloads' ? 'download' : (entityType === 'publications' ? 'publication' : 'document')));
        if (totalItems === 1) {
