...
$raw_notices = $stmt->fetchAll();

$all_documents = [];
$categoryColors = [
    'Special Notice' => 'bg-amber-50 text-amber-700 border-amber-100'
];
...
foreach ($raw_notices as $notice) {
    ...
    $all_documents[] = [
        ...
        'pdf_path_si' => !empty($notice['pdf_path_si']) ? resolvePdfUrl($notice['pdf_path_si']) : '',
        'pdf_path_ta' => !empty($notice['pdf_path_ta']) ? resolvePdfUrl($notice['pdf_path_ta']) : '',
...
<section class="py-12 md:py-16 px-4 md:px-16 bg-[#F9FAFB] min-h-[75vh]">
    <div class="container mx-auto max-w-6xl">
        ...
        <div class="bg-white rounded-2xl p-6 shadow-sm border border-gray-100 mb-8 notranslate">
            <div class="flex flex-col lg:flex-row gap-4 items-stretch lg:items-center justify-between">
                ...
                <div class="relative flex-1">
                    ...
                    <input type="text" id="searchInput" class="bg-gray-50/50 border border-gray-200 text-gray-900 text-sm rounded-xl focus:ring-secondary focus:border-secondary block w-full pl-11 pr-4 py-3 font-inter transition-all outline-none" placeholder="<?= t('search_notices_placeholder', 'Search special notices by title or date...') ?>" onkeyup="resetPaginationAndFilter()">
                </div>
        ...
        <div id="noResultsMsg" class="flex flex-col items-center justify-center py-16 px-4 bg-white rounded-3xl border border-gray-100 shadow-sm notranslate" style="display: none;">
            ...
            <p class="text-[17px] font-bold text-gray-800 mb-1"><?= t('no_notices_found', 'No notices matched your search') ?></p>
...
function filterTable() {
    ...
    if (noticesPaginator) {
        noticesPaginator.setFilteredIndexes(filteredIndexes);
    }
}
...
document.addEventListener('DOMContentLoaded', () => {
    noticesPaginator = new ContentPaginator({
        items: documents,
        entityType: 'notices',
