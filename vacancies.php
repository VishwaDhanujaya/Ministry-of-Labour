...
$pageTitle = 'Vacancies - Ministry of Labour - Sri Lanka';
$metaDescription = 'Explore current job vacancies and career opportunities at the Ministry of Labour, Sri Lanka.';
$metaKeywords = 'Vacancies, Jobs, Careers, Ministry of Labour, Sri Lanka';
...
$stmt = $pdo->query("SELECT * FROM vacancies WHERE status = 'Published' ORDER BY created_at DESC");
$raw_vacancies = $stmt->fetchAll();

$all_documents = [];
$categoryColors = [
    'Vacancy' => 'bg-indigo-50 text-indigo-700 border-indigo-100'
];
...
foreach ($raw_vacancies as $vac) {
    ...
    $all_documents[] = [
        ...
        'pdf_path_si' => !empty($vac['pdf_path_si']) ? resolvePdfUrl($vac['pdf_path_si']) : '',
        'pdf_path_ta' => !empty($vac['pdf_path_ta']) ? resolvePdfUrl($vac['pdf_path_ta']) : '',
...
function changeView(view) {
    ...
    if (vacanciesPaginator) {
        vacanciesPaginator.setView(view);
    }
}
...
function filterTable() {
    ...
    if (vacanciesPaginator) {
        vacanciesPaginator.setFilteredIndexes(filteredIndexes);
    }
}
...
document.addEventListener('DOMContentLoaded', () => {
    vacanciesPaginator = new ContentPaginator({
        items: documents,
